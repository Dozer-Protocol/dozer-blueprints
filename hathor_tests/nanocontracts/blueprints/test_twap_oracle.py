...
class TWAPOracleTestCase(BlueprintTestCase):
    ...
    def test_twap_accumulation(self):
        ...
        # Calculate expected price using PRICE_PRECISION (same as contract)
        price_a_expected = (pool.reserve_b * PRICE_PRECISION) // pool.reserve_a
        print(
            f"Initial price_a: {price_a_expected / PRICE_PRECISION:.6f} (token_b/token_a)"
        )
        ...
        self.runner.call_public_method(
            self.nc_id, "swap_exact_tokens_for_tokens", context, 30, timestamp1 + 1000
        )
        ...
        # With PRICE_PRECISION (10^8), values stay within VarUint32 range
        expected_increase = price_a_expected * 100

        # Calculate expected cumulative with modulo wrapping (for VarUint32)
        MAX_UINT32 = 2**32 - 1
        expected_cumulative = (pool.price_a_cumulative_last + expected_increase) % (
            MAX_UINT32 + 1
        )

        print(f"Expected cumulative increase: {expected_increase}")
        print(f"Expected cumulative (with modulo): {expected_cumulative}")
        print(f"Actual cumulative: {pool_after.price_a_cumulative_last}")
        print(f"Match: {expected_cumulative == pool_after.price_a_cumulative_last}")

        self.assertEqual(
            expected_cumulative,
            pool_after.price_a_cumulative_last,
            "TWAP cumulative should equal (initial + price * time_elapsed) % 2^32",
        )
    ...
    def test_twap_manipulation_resistance(self):
        ...
        for hour in range(24):
            ...
            self.runner.call_public_method(
                self.nc_id, "swap_exact_tokens_for_tokens", context, 30, ts + 1000
            )
        ...
        self.runner.call_public_method(
            self.nc_id, "swap_exact_tokens_for_tokens", context, 30, attack_ts + 1000
        )
        ...
        self.assertGreater(
            spot_change_pct, 30, "Spot price should be heavily affected by attack"
        )
    ...
    def test_no_intra_block_updates(self):
        ...
        cumulative1 = pool1.price_a_cumulative_last
        ...
        cumulative2 = pool2.price_a_cumulative_last
        ...
        self.assertEqual(
            cumulative1, cumulative2, "TWAP should not update twice in same block"
        )
