...
class DozerPoolManagerBlueprintTestCase(BlueprintTestCase):
    ...
    def test_remove_liquidity_single_token_double_withdrawal_rejected(self):
        """Two withdrawal actions for the same token must be rejected with NCFail.

        Regression test for the double-drain exploit:
        ctx.actions is dict[TokenUid, list[NCAction]], so two withdrawals for the
        same token yield len(ctx.actions)==1 but two actions in the list.
        The fix routes through _get_withdrawal_action which calls ctx.get_single_action,
        raising NCFail when len(actions) != 1.
        """
        ...
        tx = self._get_any_tx()
        # Two withdrawal actions for the same token
        actions = [
    ...
    def test_add_liquidity_single_token_double_deposit_rejected(self):
        """Two deposit actions for the same token must be rejected with NCFail."""
        pool_key, _ = self._create_pool(self.token_a, self.token_b, fee=3)

        tx = self._get_any_tx()
        # Two deposit actions for the same token
        actions = [
        ...
        with self.assertRaises(NCFail):
            self.runner.call_public_method(
                self.nc_id, "add_liquidity_single_token", context, pool_key, self.token_b
            )
    ...
    def _call_initialize_balance_tracking(self, caller_id=None):
        tx = self._get_any_tx()
        context = self.create_context(
            actions=[],
            vertex=tx,
            caller_id=caller_id or Address(self.owner_address),
            timestamp=self.get_current_timestamp(),
        )
        self.runner.call_public_method(self.nc_id, "initialize_balance_tracking", context)

    def test_initialize_balance_tracking_owner_only(self):
        """Non-owner must be rejected with Unauthorized."""
        tx = self._get_any_tx()
        non_owner = Address(self._get_any_address()[0])
        context = self.create_context(
            actions=[], vertex=tx, caller_id=non_owner, timestamp=self.get_current_timestamp()
        )
        with self.assertRaises(Unauthorized):
            self.runner.call_public_method(self.nc_id, "initialize_balance_tracking", context)

    def test_initialize_balance_tracking_populates_correctly(self):
        """After initialization get_tracked_balance must match actual reserves+total_change."""
        pool_key, _ = self._create_pool(self.token_a, self.token_b, fee=3, reserve_a=1000_00, reserve_b=1000_00)

        self._call_initialize_balance_tracking()

        contract = self.get_readonly_contract(self.nc_id)
        assert isinstance(contract, DozerPoolManager)
        pool = contract.pools[pool_key]

        tracked_a = self.runner.call_view_method(self.nc_id, "get_tracked_balance", self.token_a)
        tracked_b = self.runner.call_view_method(self.nc_id, "get_tracked_balance", self.token_b)

        self.assertEqual(tracked_a, pool.reserve_a + pool.total_change_a)
        self.assertEqual(tracked_b, pool.reserve_b + pool.total_change_b)

    def test_tracked_balance_maintained_after_swap(self):
        """Tracked balance must stay in sync after a swap."""
        pool_key, _ = self._create_pool(self.token_a, self.token_b, fee=3)
        self._call_initialize_balance_tracking()

        # Compute expected output and perform a swap
        reserve_a, reserve_b = self.runner.call_view_method(
            self.nc_id, "get_reserves", self.token_a, self.token_b, 3
        )
        amount_out = self.runner.call_view_method(
            self.nc_id, "get_amount_out", 100_00, reserve_a, reserve_b, 3, 1000
        )
        _result, _ = self._swap_exact_tokens_for_tokens(self.token_a, self.token_b, 3, 100_00, amount_out)
    ...
    def test_tracked_balance_maintained_after_liquidity_ops(self):
        """Tracked balance stays correct after add and remove liquidity."""
        pool_key, _ = self._create_pool(self.token_a, self.token_b, fee=3, reserve_a=100000_00, reserve_b=100000_00)
        self._call_initialize_balance_tracking()
