...
class MVP_PoolBlueprintTestCase(BlueprintTestCase):
    def setUp(self):
        ...
        self.nc_catalog.blueprints[self.blueprint_id] = Dozer_Pool_v1_1
    ...
    def _prepare_remove_liquidity_context(self, amount_a, amount_b, address=None):
        actions = [
    ...
    def test_remove_liquidity_no_change(self) -> Context:
        ...
        ctx = self._prepare_remove_liquidity_context(
            amount_a, amount_b, ctx_add.address
        )
    ...
    def test_add_swap_remove(self) -> None:
        ...
        ctx_remove = self._prepare_remove_liquidity_context(
            amount_a_remove, amount_b_remove, address_user_with_liquidity
        )
    ...
    def test_multiple_add_and_remove_liquidity(self) -> None:
        ...
        for i in range(users):
            ...
            ctx = self._prepare_remove_liquidity_context(
                amount_a, amount_b, ctx_adds[i].address
            )
            self.runner.call_public_method(self.nc_id, "remove_liquidity", ctx)
    ...
    def test_multiple_add_swap_and_remove_liquidity(self) -> None:
        ...
        for i in range(users):
            ...
            liquidity_increase = (
                total_liquidity * (protocol_fee_amount) // (2 * reserve_a)
            )
            self.assertEqual(
                liquidity_increase,
                self.runner.call_view_method(
        ...
        for i in range(users):
            ...
            ctx = self._prepare_remove_liquidity_context(
                remove_amount_a, remove_amount_b, ctx_adds[i].address
            )
            self.runner.call_public_method(self.nc_id, "remove_liquidity", ctx)
    ...
    def test_add_swap_remove_liquidity_random(self) -> None:
        ...
        ctx_remove = self._prepare_remove_liquidity_context(
            remove_amount_a, remove_amount_b, ctx_add.address
        )
        self.runner.call_public_method(self.nc_id, "remove_liquidity", ctx_remove)
    ...
    def test_random_user_interactions(self):
        ...
        for _ in range(num_actions):
            ...
            if action == "add_liquidity":
                ...
                if user_liquidity > 0:
                    ...
                    ctx_remove = self._prepare_remove_liquidity_context(
                        remove_amount_a, remove_amount_b, user
                    )
        ...
        print("total_user_liquidity:", int(total_user_liquidity))
        print("admin_liquidity:", int(admin_liquidity))
        print("final_total_liquidity:", int(final_total_liquidity))
        print(
            "total_user_liquidity + admin_liquidity:",
            int(total_user_liquidity) + int(admin_liquidity),
        )
        self.assertEqual(
            (int(total_user_liquidity) + int(admin_liquidity)),
            int(final_total_liquidity),
        )
