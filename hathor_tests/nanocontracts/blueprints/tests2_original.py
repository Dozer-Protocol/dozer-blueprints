...
from hathor_tests.nanocontracts.dozer.dozer_pool_manager import (
    DozerPoolManager,
    InvalidAction,
    InvalidFee,
    InvalidState,
...
class TestDozerPoolManager(BlueprintTestCase):
    ...
    def test_pause(self) -> None:
        ...
        with pytest.raises(InvalidState, match=msg):
            self.runner.call_public_method(self.contract_id, 'withdraw_cashback', self.create_context(), fee=0)
    ...
    def test_add_liquidity_single_token(self) -> None:
        ...
        assert quote.token_a_used == 66
        assert quote.token_b_used == 151
        assert quote.excess_token == self.token_b.hex()
        assert quote.swap_amount == 144
        assert quote.swap_output == 66
        ...
        assert state.reserve_b == 2295
        assert state.total_change_a == 0
        assert state.total_change_b == 5
    ...
    def test_remove_liquidity_single_token(self) -> None:
        ...
        assert quote.amount_out == 362889
        assert quote.swap_amount == 99999
        assert quote.swap_output == 162890
        assert quote.token_a_withdrawn == 99999
        assert quote.token_b_withdrawn == 199999

        ctx = self.create_context(
            actions=[NCWithdrawalAction(token_uid=self.token_b, amount=100000)], caller_id=creator
        )
        ...
        assert state.reserve_a == 1000000
        assert state.reserve_b == 2000000 - 362889
        assert state.total_change_a == 0
        assert state.total_change_b == 362889 - 100000
