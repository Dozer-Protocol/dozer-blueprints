...
from hathor.nanocontracts.storage.factory import NCMemoryStorageFactory
from hathor.nanocontracts.storage.patricia_trie import PatriciaTrie
from hathor.nanocontracts.types import NCAction, NCActionType, ContractId
...
class OasisTestCase(BlueprintTestCase):
    ...
    def setUp(self):
        ...
        self.runner = TestRunner(
            self.manager.tx_storage, nc_storage_factory, block_trie
        )

        # Set up Oasis contract
        self.oasis_id = ContractId(b"2" * 32)
        ...
        self.dozer_id = ContractId(b"1" * 32)
    ...
    def _initialize_oasis(self, amount: int = 10_000_000_00):
        """Initialize Oasis"""
        ctx = Context(
            [
    ...
    def _initialize_pool(self, amount_htr: int = 1000000, amount_b: int = 7000000):
        """Initialize Dozer Pool"""
        ...
        pool_ctx = Context(actions, self._get_any_tx(), self.dev_address, timestamp=self._get_current_timestamp())  # type: ignore
        self.runner.call_public_method(
            self.dozer_id,
            "initialize",
            pool_ctx,
    ...
    def test_user_withdraw_exact_value(self):
        ...
        deposit_amount = ctx_deposit.actions.get(self.token_b).amount  # type: ignore
