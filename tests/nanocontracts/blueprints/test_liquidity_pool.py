...
class LiquidityPoolBlueprintTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.runner = Runner(LiquidityPool, b'', self.nc_storage)
    ...
    def _initialize_contract(self, reserve_a, reserve_b):
        ...
        self.runner.call_public_method('initialize', context, self.token_a, self.token_b, self.lp_token, 0)
    ...
    def _swap1(self, token_in, amount_in, token_out, amount_out):
        ...
        result = self.runner.call_public_method('swap_exact_tokens_for_tokens', context, context.address)
        return result, context

    def _swap2(self, token_in, amount_in, token_out, amount_out):
        context = self._prepare_swap_context(token_in, amount_in, token_out, amount_out)
        result = self.runner.call_public_method('swap_tokens_for_exact_tokens', context, context.address)
        return result, context
    ...
    def test_swap1_no_change(self) -> None:
        ...
        amount_in = 20_00
        amount_out = self.runner.call_private_method('get_amount_out', amount_in, reserve_a, reserve_b)
        ...
        self.assertEqual(0, storage.get('total_balance_a'))
        self.assertEqual(0, storage.get('total_balance_b'))

        self.assertEqual((0, 0), self.runner.call_private_method('balance_of', context.address))
    ...
    def test_swap1_with_change(self) -> None:
        ...
        self.assertEqual(0, storage.get('total_balance_a'))
        self.assertEqual(1, storage.get('total_balance_b'))

        self.assertEqual((0, change), self.runner.call_private_method('balance_of', context.address))
        self.assertBalanceReserve(storage)
    ...
    def test_swap2_no_change(self) -> None:
        ...
        amount_out = 20_00
        amount_in = self.runner.call_private_method('get_amount_in', amount_out, reserve_a, reserve_b)
    ...
    def test_swap2_with_change(self) -> None:
        ...
        self.assertEqual(1, storage.get('total_balance_a'))
        self.assertEqual(0, storage.get('total_balance_b'))

        self.assertEqual((change, 0), self.runner.call_private_method('balance_of', context.address))
        self.assertBalanceReserve(storage)
    ...
    def test_amount_in_out(self) -> None:
        ...
        self.assertEqual(19_60, self.runner.call_private_method('get_amount_out', 20_00, 1_000_00, 1_000_00))
        self.assertEqual(20_40, self.runner.call_private_method('get_amount_in', 20_00, 1_000_00, 1_000_00))
    ...
    def _prepare_add_liquidity_context(self, amount_a, amount_b):
        ...
        return Context(actions, self._get_any_tx(), self._get_any_address(), timestamp=self.get_current_timestamp())
    ...
    def test_add_liquidity_no_change(self) -> None:
        ...
        amount_a = 100_00
        amount_b = self.runner.call_private_method('quote', amount_a, reserve_a, reserve_b)

        ctx = self._prepare_add_liquidity_context(amount_a, amount_b)
        self.runner.call_public_method('add_liquidity', ctx, amount_a, amount_b, ctx.address)
    ...
    def test_add_liquidity_change_a(self) -> None:
        ...
        ctx = self._prepare_add_liquidity_context(amount_a, amount_b)
        self.runner.call_public_method('add_liquidity', ctx, amount_a_min, amount_b_min, ctx.address)
        ...
        self.assertEqual(change, storage.get('total_balance_a'))
        self.assertEqual(0, storage.get('total_balance_b'))

        self.assertEqual((change, 0), self.runner.call_private_method('balance_of', ctx.address))
        self.assertBalanceReserve(storage)
    ...
    def test_add_liquidity_change_b(self) -> None:
        ...
        self.assertEqual(0, storage.get('total_balance_a'))
        self.assertEqual(change, storage.get('total_balance_b'))

        self.assertEqual((0, change), self.runner.call_private_method('balance_of', ctx.address))
        self.assertBalanceReserve(storage)