...
class DozerToolsTest(BlueprintTestCase):
    ...
    def test_crowdsale_successful_lifecycle(self) -> None:
        ...
        for deposit_amount in deposit_amounts:
            ...
            participate_ctx = Context(
                [NCDepositAction(token_uid=htr_uid, amount=deposit_amount)],
                self._get_any_tx(),
                Address(user_addr),
        ...
        for user_addr, deposit_amount in participants:
            ...
            claim_ctx = Context(
                [NCWithdrawalAction(token_uid=token_uid, amount=tokens_due)],
                self._get_any_tx(),
                Address(user_addr),
        ...
        owner_withdraw_ctx = Context(
            [NCWithdrawalAction(token_uid=htr_uid, amount=withdrawable_htr)],
            self._get_any_tx(),
            self.dev_address,
    ...
    def test_crowdsale_failed_sale_with_refunds(self) -> None:
        ...
        for user_addr, deposit_amount in participants:
            ...
            # Claim refund
            refund_ctx = Context(
                [NCWithdrawalAction(token_uid=htr_uid, amount=deposit_amount)],
                self._get_any_tx(),
                Address(user_addr),
                timestamp=end_time + 200,
            )
    ...
    def test_crowdsale_pause_unpause_operations(self) -> None:
        ...
        participate_paused_ctx = Context(
    ...
    def test_crowdsale_edge_cases(self) -> None:
        ...
        below_min_ctx = Context(
            [NCDepositAction(token_uid=htr_uid, amount=50_00)],  # Below 100 HTR
            self._get_any_tx(),
            Address(user_addr1),
        ...
        first_deposit_ctx = Context(
            [NCDepositAction(token_uid=htr_uid, amount=first_deposit)],
            self._get_any_tx(),
            Address(user_addr2),
        ...
        second_deposit_ctx = Context(
            [NCDepositAction(token_uid=htr_uid, amount=second_deposit)],
            self._get_any_tx(),
            Address(user_addr2),
        ...
        exceed_ctx = Context(
            [NCDepositAction(token_uid=htr_uid, amount=exceed_hardcap_amount)],
            self._get_any_tx(),
            Address(user_addr3),
        ...
        exact_remaining_ctx = Context(
            [NCDepositAction(token_uid=htr_uid, amount=remaining)],
            self._get_any_tx(),
            Address(user_addr3),
            timestamp=start_time + 50,
        )
    ...
    def test_crowdsale_routed_admin_methods(self) -> None:
        ...
        user_addr, _ = self._get_any_address()
        participate_ctx = Context(
            [NCDepositAction(token_uid=htr_uid, amount=1000_00)],
            self._get_any_tx(),
            Address(user_addr),
            timestamp=start_time + 50,
        )
    ...
    def test_crowdsale_routed_withdrawal_methods(self) -> None:
        ...
        user_addr, _ = self._get_any_address()
        participate_ctx = Context(
            [NCDepositAction(token_uid=htr_uid, amount=600_00)],
            self._get_any_tx(),
            Address(user_addr),
            timestamp=start_time + 50,
        )
        ...
        withdraw_tokens_ctx = Context(
            [NCWithdrawalAction(token_uid=token_uid, amount=remaining_tokens)],
            self._get_any_tx(),
            self.dev_address,
            timestamp=end_time + 200,
        )
    ...
    def test_crowdsale_unauthorized_routed_calls(self) -> None:
        ...
        unauthorized_pause_ctx = Context(
            [],
            tx,
            Address(unauthorized_addr),
            timestamp=start_time + 50,
        )
