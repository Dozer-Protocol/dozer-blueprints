...
class MyBlueprint(Blueprint):
    ...
    def get_tokens_from_another_contract(self, ctx: Context) -> None:
        ...
        for action in ctx.actions.values():
            ...
            if diff < 0:
                actions.append(
                    NCAction(NCActionType.WITHDRAWAL, action.token_uid, -diff)
                )

        if actions:
            ctx.call_public_method(
                self.contract, "get_tokens_from_another_contract", actions
            )
    ...
    def dec(self, ctx: Context, fail_on_zero: bool = True) -> None:
        ...
        if self.contract:
            ...
            ctx.call_public_method(
                self.contract, "dec", actions, fail_on_zero=fail_on_zero
            )
...
class NCBlueprintTestCase(unittest.TestCase):
    ...
    def test_getting_funds_from_another_contract(self):
        ...
        self.runner.call_public_method(
            self.nc1_id, "get_tokens_from_another_contract", ctx
        )
    ...
    def test_transfer_between_contracts(self):
        ...
        total_counter = self.runner.call_public_method(
            self.nc1_id, "get_total_counter", ctx
        )
        self.assertEqual(total_counter, 21)

        total_counter = self.runner.call_public_method(
            self.nc2_id, "get_total_counter", ctx
        )
