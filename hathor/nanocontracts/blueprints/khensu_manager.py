...
class KhensuManager(Blueprint):
    ...
    def migrate_liquidity(self, token_uid: TokenUid) -> None:
        ...
        initial_virtual_pool = self.token_initial_virtual_pool[token_uid]
        market_cap = virtual_pool - initial_virtual_pool
        ...
        if market_cap < liquidity_amount + self.graduation_fee:
        ...
        # TODO: Check if it is token_reserve or if it shuold be minted.
        self.collected_graduation_fees += market_cap - liquidity_amount
    ...
    def buy_tokens(self, ctx: Context, token_uid: TokenUid) -> None:
        ...
        virtual_pool = self.token_virtual_pools.get(token_uid)
        initial_virtual_pool = self.token_initial_virtual_pool.get(token_uid)
        market_cap = virtual_pool - initial_virtual_pool
        target_market_cap = self.token_target_caps.get(token_uid)
        max_net_amount = target_market_cap - market_cap
        ...
        if (
            self.token_virtual_pools[token_uid]
            - self.token_initial_virtual_pool[token_uid]
            >= self.token_target_caps[token_uid]
    ...
    def quote_buy(self, token_uid: TokenUid, htr_amount: Amount) -> dict[str, int]:
        ...
        tokens_out = self._calculate_tokens_out(token_uid, net_amount)
        tokens_to_graduate = self._calculate_tokens_out(token_uid, max_net_amount)
        # Garantee that the market cap will be met in order to graduate:
        if tokens_to_graduate == self._calculate_tokens_out(
            token_uid, max_net_amount - 1
        ):
            tokens_to_graduate += 1
        ...
        if tokens_out >= tokens_to_graduate:
            ...
            tokens_theoretical = self._calculate_tokens_out(
                token_uid, max_net_amount + fee_amount
            )
